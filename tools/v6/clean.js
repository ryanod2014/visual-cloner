 ...
 *
 * Key difference from v6-complete.js:
 * - Does NOT click "Start using Photopea"
 * - Does NOT trigger keyboard shortcuts or menu clicks
 * - Captures the LANDING PAGE HTML, not editor state
 * - Resources are loaded by the JS when served
 */
...
async function main() {
  ...
  console.log('V6 CLEAN - Resource Capture (NO DOM Modification)');
  console.log('='.repeat(60));
  console.log('URL:', url);
  ...
  try {
    ...
    // NO exhaustFeatures() - don't trigger menus/shortcuts that create duplicates
  ...
  // Find and patch r9.js (main Photopea code)
  let patchedR9 = null;
  ...
  if (ppJsEntry) {
    ...
    // NEW PATTERN (2025+): lm variable at beginning
    // var lm=window.location.hostname.endsWith("jampea.com")?2:window.location.hostname=="www.vectorpea.com"?1:0
    const lmPattern = /var lm=window\.location\.hostname\.endsWith\("jampea\.com"\)\?2:window\.location\.hostname=="www\.vectorpea\.com"\?1:0/;
    if (lmPattern.test(r9Content)) {
      r9Content = r9Content.replace(lmPattern, 'var lm=1');
      console.log('  PATCH (new): lm = 1 (bypasses all lm==0 checks)');
      patchApplied = true;
    }

    // OLD PATTERN: J.adQ() function
  ...
  http.createServer((req, res) => {
    ...
    console.log('');
    console.log('='.repeat(60));
    console.log('SERVER READY (with patches)');
    console.log('='.repeat(60));
