...
function isObfuscatedName(name) {
  ...
  // Skip things that look like hex color codes (must be all lowercase or include numbers that aren't just trailing)
  // But DON'T skip things like D2, A3, F5 which are obfuscated variable names
  if (/^[0-9a-f]+$/i.test(name) && name.length >= 3 && name.length <= 6 && /\d.*\d/.test(name)) return false;
...
function findObfuscatedIdentifiers(code) {
  ...
  if (process.env.DEBUG) {
    console.log('DEBUG: All obfuscated identifiers found:');
    for (const [name, count] of identifiers) {
      console.log(`  ${name}: ${count}x`);
    }
  }

  // Filter to only those appearing minOccurrences+ times (high-value targets)
  const minOccurrences = parseInt(process.env.MIN_OCCURRENCES || '3', 10);
